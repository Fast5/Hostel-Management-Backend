                        ...
                        if(result){
                            jwt.sign({username: foundUser.username, id: foundUser._id, role: "admin"}, process.env.SECRET, {}, function(err, token){
                                if(err){
                                    ...
                                    res.status(200).cookie("token", token, {secure: true, sameSite: "none", path: "/", domain: ".hostel-management-frontend-plum.vercel.app"}).json({"user": foundUser, "success": "Login successful."});
                                }
                            });
                        }
                        else{
